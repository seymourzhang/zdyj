...
function createTableColumns(){
    var dataColumns = [
                       ...
                       [{field: "farm_name",
                            ...
                            formatter: function(value,row,index){
                            	return colorSelect(value,row.low_alarm_temp,row.high_alarm_temp,'onclick="forwardMenu(\'z201\',\'lm2\',\'se2\',\'op2\',\'环境监测分析\',\'/surroundingsReport/showSurroundingsReport?farm_id=' + row.farm_id + '&house_id=' + row.house_id  + '&batch_no=' + row.batch_no +'&tabId=0' +'&org_id='+row.corporation_id + '\')"');
//                            	if (value == -99) {
...
//                },
                        ...
                        formatter: function(value,row,index){
                        	return colorSelect(value,'-',row.high_alarm_co2,'onclick="forwardMenu(\'z204\',\'lm2\',\'se2\',\'op2\',\'环境监测分析\',\'/surroundingsReport/showSurroundingsReport?farm_id=' + row.farm_id + '&house_id=' + row.house_id  + '&batch_no=' + row.batch_no +'&tabId=2' +'&org_id='+row.corporation_id +  '\')"');
//                        	if (row.co2 == 0 || row.co2 == -99) {
...
//                    },
                            ...
                            formatter: function(value,row,index){
                            	...
                            	return colorSelect(value,row.low_lux,row.high_lux,'onclick="forwardMenu(\'z207\',\'lm2\',\'se2\',\'op2\',\'环境监测分析\',\'/surroundingsReport/showSurroundingsReport?farm_id=' + row.farm_id + '&house_id=' + row.house_id  + '&batch_no=' + row.batch_no +'&tabId=3' +'&org_id='+row.corporation_id +  '\')"');
//                            	if (row.lux == -99) {
...
function colorSelect(mb,db,gb,h1){
	if (mb == '-') {
    	...
    	if(db == '-'){
            ...
            }else if(parseFloat(mb)-parseFloat(db) >0 && parseFloat(mb)-parseFloat(db) <=1){
    	...
    	if (parseFloat(mb)>parseFloat(gb)) {
    		return '<a href="#" style="color: #E83828;" '+h1+'>'+mb+'</a>';
        }else if(parseFloat(gb)-parseFloat(mb) >0 && parseFloat(gb)-parseFloat(mb) <=1){
        	return '<a href="#" style="color: #f5a623;" '+h1+'>'+mb+'</a>';
        }
         else{
        	return '<a href="#" '+h1+'>'+mb+'</a>';
    	...
    	if (parseFloat(mb)<parseFloat(db) || parseFloat(mb)>parseFloat(gb)) {
    		return '<a href="#" style="color: #E83828;" '+h1+'>'+mb+'</a>';
        		...
        		|| (parseFloat(gb)-parseFloat(mb) >0 && parseFloat(gb)-parseFloat(mb) <=1)){
        	return '<a href="#" style="color: #f5a623;" '+h1+'>'+mb+'</a>';
        }
         else{
        	return '<a href="#" '+h1+'>'+mb+'</a>';
